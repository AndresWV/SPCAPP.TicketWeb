using System;
using System.Collections.Generic;

#nullable disable

namespace SPCAPP.TicketWeb.Models
{
    public partial class Auxi1
    {
        public string CodAux { get; set; }
        public string NomAux { get; set; }
        public string NoFaux { get; set; }
        public string RutAux { get; set; }
        public string ActAux { get; set; }
        public string GirAux { get; set; }
        public string ComAux { get; set; }
        public string CiuAux { get; set; }
        public string PaiAux { get; set; }
        public string ProvAux { get; set; }
        public string DirAux { get; set; }
        public string DirNum { get; set; }
        public string FonAux1 { get; set; }
        public string FonAux2 { get; set; }
        public string FonAux3 { get; set; }
        public string FaxAux1 { get; set; }
        public string FaxAux2 { get; set; }
        public string ClaCli { get; set; }
        public string ClaPro { get; set; }
        public string ClaEmp { get; set; }
        public string ClaSoc { get; set; }
        public string ClaDis { get; set; }
        public string ClaOtr { get; set; }
        public string DiaPlazo { get; set; }
        public string Bloqueado { get; set; }
        public string Email { get; set; }
        public string Casilla { get; set; }
        public string WebSite { get; set; }
        public string Notas { get; set; }
        public int? Region { get; set; }
        public int? TipoSaludo { get; set; }
        public string DirDpto { get; set; }
        public string DirOtro { get; set; }
        public int? CodPostal { get; set; }
        public int? CodAreaFon { get; set; }
        public int? AnexoFon { get; set; }
        public int? CodAreaFax { get; set; }
        public DateTime? FechaNacim { get; set; }
        public string Username { get; set; }
        public string Password { get; set; }
        public string PalabraSecreta { get; set; }
        public string PreguntaSecreta { get; set; }
        public DateTime? ClienteDesde { get; set; }
        public int? TipoUsuario { get; set; }
        public string EMailDte { get; set; }
        public string EsReceptorDte { get; set; }
        public string BloqueadoPro { get; set; }
        public string ClaPros { get; set; }
        public int? CodCamp { get; set; }
        public int? CodOrigen { get; set; }
        public string IdRecepExtranjero { get; set; }
        public string PaisRecepExtranjero { get; set; }
        public string Usuario { get; set; }
        public string Sistema { get; set; }
        public string Proceso { get; set; }
        public DateTime? FechaUlMod { get; set; }
        public string CtaCliente { get; set; }
        public string CtaCliMonExt { get; set; }
    }
}
